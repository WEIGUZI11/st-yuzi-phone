...
export function buildFusionPageHtml() {
    const navigationHtml = buildPhoneNavBar({
        ...
        centerHtml: buildPhoneNavTitleSwitcher({ title: t("模板缝合") }),
    });
    return `
        <div class="phone-app-page phone-fusion-page">
            ...
            <div class="phone-app-body phone-fusion-body">
                <div class="phone-fusion-desc">
                    ${t`从内置模板、本地 JSON 或当前数据库表格中选择区块，自动合并为新模板`}
                </div>
                <div class="phone-fusion-import-row">
                    <div class="phone-fusion-import-card" id="phone-source-a">
                        ${PHONE_ICONS.upload}
                        <span>${t`模板 A 来源`}</span>
                        <button type="button" class="phone-fusion-source-action" id="phone-use-builtin-a">${t`使用内置小剧场+纪要表`}</button>
                        <button type="button" class="phone-fusion-source-action" id="phone-import-a">${t`导入本地 JSON`}</button>
                        <span class="phone-fusion-file-name" id="phone-fname-a"></span>
                    </div>
                    <div class="phone-fusion-import-card" id="phone-source-b">
                        ${PHONE_ICONS.upload}
                        <span>${t`模板 B 来源`}</span>
                        <button type="button" class="phone-fusion-source-action" id="phone-use-current-db-b">${t`选择当前表格`}</button>
                        <button type="button" class="phone-fusion-source-action" id="phone-import-b">${t`导入本地 JSON`}</button>
                ...
                <div id="phone-fusion-actions" class="phone-fusion-actions" style="display:none;">
                    ...
                    <button type="button" class="phone-fusion-secondary-btn" id="phone-fusion-import-template" title="${t`将合并结果导入为数据库模板/预设`}">
                        <span>${t`导入为模板/预设`}</span>
                    </button>
                </div>
    ...
    return `
        <div class="phone-fusion-table">
            <div class="phone-fusion-table-header">
                ...
                <span class="phone-fusion-col-name">${t`表格名称`}</span>
                <span class="phone-fusion-col-source">${t`来源`}</span>
                <span class="phone-fusion-col-cols">${t`列数`}</span>
            </div>
            ${rowsHtml}
        </div>
    `;
...
export function buildFusionEmptyResultHtml() {
    return `<div class="phone-empty-msg">${t`未选中任何表格`}</div>`;
}
    ...
    return `
        <div class="phone-fusion-success">
            ...
            <div class="phone-fusion-success-text">
                <strong>${t`合并完成`}</strong>
                <span>${t`${sheetCount} 个表格已合并`}</span>
            </div>
