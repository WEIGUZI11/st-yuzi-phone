...
export function buildVariableManagerPageHtml(messageId, isMvu) {
    const floorLabel = messageId >= 0 ? t`第${messageId}楼` : t("无数据");
    ...
    const navigationHtml = buildPhoneNavBar({
        ...
        centerHtml: `
            <div class="phone-nav-title vm-nav-title">
                <span class="vm-nav-title-text">${t`变量管理器`}</span>
                ${mvuBadge}
            </div>
        `,
        trailingHtml: `
            <div class="vm-nav-right-group">
                ...
                <button type="button" class="vm-nav-refresh" data-vm-action="refresh" aria-label="${t`刷新`}">
    ...
    return `
        <div class="phone-app-page vm-page">
            ...
            <div class="vm-delete-bar vm-delete-bar-hidden" data-phone-bottom-bar>
                <button type="button" class="vm-delete-cancel-btn" data-vm-action="exit-delete-mode">${t`完成`}</button>
                <button type="button" class="vm-delete-confirm-btn" data-vm-action="confirm-delete">
                    ...
                    <span>${t`删除选中 (`}<span class="vm-delete-count">0</span>)</span>
                </button>
            </div>
        </div>
    `;
        ...
        <div class="vm-card vm-card-editing" data-var-path="${escapeHtmlAttr(path)}" data-var-type="${escapeHtmlAttr(valueType)}">
            <div class="vm-card-edit-inner">
                ...
                <div class="vm-edit-actions">
                    <button type="button" class="vm-edit-cancel" data-vm-action="cancel-edit">${t`取消`}</button>
                    <button type="button" class="vm-edit-save" data-vm-action="save-edit">${t`保存`}</button>
                </div>
            </div>
        </div>
    `;
...
export function buildAddVariableDialogHtml() {
    return `
        <div class="vm-dialog-overlay">
            <div class="vm-dialog">
                <div class="vm-dialog-title">${t`添加变量`}</div>
                <div class="vm-dialog-body">
                    <div class="vm-dialog-field">
                        <label class="vm-dialog-label">${t`变量路径`}</label>
                        <input type="text" class="vm-dialog-input vm-add-path-input" placeholder="${t`例如: 角色.络络.好感度`}" autocomplete="off" spellcheck="false">
                        <div class="vm-dialog-hint">${t`使用 . 分隔层级`}</div>
                    </div>
                    <div class="vm-dialog-field">
                        <label class="vm-dialog-label">${t`变量值`}</label>
                        <input type="text" class="vm-dialog-input vm-add-value-input" placeholder="${t`例如: 30`}" autocomplete="off" spellcheck="false">
                        <div class="vm-dialog-hint">${t`数字、字符串、true/false、null、JSON数组/对象`}</div>
                    </div>
                </div>
                <div class="vm-dialog-buttons">
                    <button type="button" class="vm-dialog-btn vm-dialog-cancel" data-vm-dialog-action="cancel">${t`取消`}</button>
                    <button type="button" class="vm-dialog-btn vm-dialog-confirm" data-vm-dialog-action="confirm">${t`添加`}</button>
                </div>
            </div>
        </div>
    `;
 ...
 * 确认弹窗 HTML
 */
export function buildConfirmDialogHtml(title, bodyHtml, confirmText = t("确认"), confirmClass = '') {
    return `
        <div class="vm-dialog-overlay">
            <div class="vm-dialog">
