...
const LIVE_BARRAGE_FIELDS = Object.freeze([
    Object.freeze({ header: '剧情弹幕串', kind: 'plot' }),
    Object.freeze({ header: '推角弹幕串', kind: 'stan' }),
    Object.freeze({ header: '对线弹幕串', kind: 'clash' }),
]);
...
function buildBarrageItem({ text, kind, badge, time, bandIndex, itemIndex }) {
    const safeText = normalizeText(text);
    if (!safeText) return null;
    const safeBadge = normalizeText(badge) || DEFAULT_BARRAGE_BADGE;
    const safeKind = BARRAGE_KIND_LABELS[kind] ? kind : 'plot';
    const seed = `${safeBadge}|${safeKind}|${safeText}|${bandIndex}|${itemIndex}`;
    return {
        text: safeText,
        badge: safeBadge,
        kind: safeKind,
...
function parseLiveBarrages(value, options = {}) {
    ...
    const requestedKind = normalizeText(options.kind);
    ...
    return splitSemicolonText(text)
        ...
        .map((segment, itemIndex) => buildBarrageItem({
            text: segment.text,
            kind: BARRAGE_KIND_LABELS[requestedKind] ? requestedKind : resolveBarrageKind(`${segment.badge}|${segment.text}`, itemIndex),
            badge: segment.badge,
            time,
            bandIndex,
            itemIndex,
...
function collectLiveBarrageItems(roomsTable, row, options = {}) {
    return LIVE_BARRAGE_FIELDS.flatMap((field, fieldIndex) => parseLiveBarrages(getCellByHeader(roomsTable, row, field.header), {
        ...options,
        kind: field.kind,
        bandIndex: Number.isInteger(options.bandIndex)
            ? options.bandIndex + fieldIndex
            : fieldIndex,
    }));
}
...
function buildViewModel(resolved) {
    ...
    const rooms = mapTheaterRows(roomsTable, (row, rowIndex) => {
        ...
        return {
            ...
            time,
            barrageItems: collectLiveBarrageItems(roomsTable, row, {
                time,
                bandIndex: rowIndex,
            }),
        };
...
function renderLiveStatusStrip(room) {
    const metricsHtml = renderMetricPills(room);
    if (!metricsHtml) return '';
    return `
        <section class="phone-theater-live-status-strip" aria-label="直播状态">
            <div class="phone-theater-live-stats">${metricsHtml}</div>
        </section>
    `;
}
...
function renderLivePoster(room) {
    ...
    const statusHtml = room.castTag
        ? `<span class="phone-theater-live-poster-status">${escapeHtml(room.castTag)}</span>`
        : '';
    return `
...
function renderBarrageItem(item) {
    ...
    return `
        <li class="phone-theater-live-barrage-item ${kindClass} ${toneClass}" data-indent="${indent}">
