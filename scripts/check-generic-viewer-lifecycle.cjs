...
function main() {
    ...
    assertIncludes(addRowModal, "if (!isViewerActive()) return;\n            showInlineToast(container, t`新增异常: ${err?.message || t(\"未知错误\")}`);", 'add-row-modal catch 分支 disposed 后不 toast/恢复旧按钮', failures);
    ...
    assertIncludes(rowDelete, "const synced = syncRowsFromSheet();\n        const message = result.message || (deletedRowIndexes.length > 1 ? t`已删除 ${deletedRowIndexes.length} 条记录` : t(\"删除成功\"));", 'row-delete-controller active 成功路径才同步当前视图', failures);
