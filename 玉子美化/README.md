...
| `npm run project:add-qq` | 登记 QQ 主题、通知浮窗或人物装饰；支持 `--dry-run` |
...
先问用户要干什么时，必须同时问“要不要美化 QQ 主题或 QQ 通知浮窗”。主题和通知浮窗分开确认；用户拒绝就不用管，同意才进入对应制作。人物装饰只在用户明确要做时进入，不额外强迫所有人回答一轮素材问题。做主题时只顺带问一次要不要分别做白天／夜间两种外观，不强制。本轮不建设 QQ 专用模拟器，也不设置 QQ 模拟确认关卡；自动检查通过后，真实效果在真实小手机中验收。涉及 QQ 时先读取 `docs/runtime/qq-authoring-workflow.md`，使用 `project:add-qq` 登记。
